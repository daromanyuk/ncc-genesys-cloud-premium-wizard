...
export default {
     ...
     */
    showProductUnavailable(noPermission){
        let elAvailable = document.getElementById('available');
        let elUnavailable = document.getElementById('unavailable');
        elAvailable.style.display = 'none';
        elUnavailable.style.display = '';
        if(noPermission) {
            // set no permission text
            let productNotAvailable = document.getElementById("product-not-available");
            productNotAvailable.textContent = "You do not have \"employee\" permission.";
        }
    },  
